...
import jmetal.util.comparators.A_DominanceComparator;
...
public class A_Ranking {
   ...
   */
  public A_DominanceComparator dominance_ = new A_DominanceComparator(); // dominance.compare(,,extre)
   ...
   */       
  public A_Ranking(SolutionSet solutionSet, int[] weight, double[] alpha) {        
    ...
    // of individuals that dominate me
      ...
      for (int q = 0; q < solutionSet_.size(); q++) {
        ...
        if (flagDominate == 0) {
           weightp = weight[p];
           weightq = weight[q];
           alphax = alpha[weight[p]];
          flagDominate =dominance_.compare(solutionSet.get(p),solutionSet.get(q),weightp,weightq,alphax); // extreme                           
        }
        if (flagDominate == -1) {
