 ...
 */
public class KneeDominanceComparator2 implements Comparator{
  ...
  */
   ...
   * @return
   */
  public int compare(Solution A, Solution B, double[][] extremex, double gama, int weight1, int weight2,int[] countindex2, int eva, int maxeva, int popsize)  {
	...
	// -1 : solution1 better; 1 solution2 better; 0 equal.
	  ...
	  if(weight1==weight2) {
		  // tuning gama
		  gama = tuningGama(weight1, countindex2, eva, maxeva, popsize);		  
  ...
  public double tuningGama(int weightindex, int[] countindex2, int eva, int maxeva, int popsize) {
	double temp1 = 0.1*eva/(0.1*maxeva);
	double temp2 = 0.1*countindex2[weightindex]/(0.2*popsize);
	double gama = 0.5;
	
	if(temp1<temp2 || temp1<0.5) {
		gama = 0.5;
	}else {
		gama = temp1-temp2;
	}
