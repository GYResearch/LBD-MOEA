 ...
 * @throws JMException 
  ...
  */
   ...
   */
   public void crowdingDistanceAssignment2(double[][] extre, SolutionSet solutionSet, int nObjs) {
...
//   // calc the crowding distance of the front.get(0)
   double dis1 = 0.0, dis2=0;
   for(int k=0;k<nObjs;k++) {
	   // the dist for front.get(0)
	   dis1 = Math.abs(extre[1][k] - front.get(1).getObjective(k));                    
	   dis1 = dis1 / (extre[1][k] - extre[0][k]);    
	   dis1 += dis1;            
	   front.get(0).setCrowdingDistance(dis1);
	   // the dist for front.get(size-1)
	   dis2 = Math.abs(extre[0][k] - front.get(size-2).getObjective(k));                    
	   dis2 = dis2 / (extre[1][k] - extre[0][k]);        
	   dis2 += dis2;
       front.get(size-1).setCrowdingDistance(dis2); 
   }
     
 // calc the crowding distance    
     for (int i = 0; i<nObjs; i++) {          
       // Sort the population by Obj n            
       front.sort(new ObjectiveComparator(i));
       objetiveMinn = extre[0][i];   // ideal point
       objetiveMaxn = extre[1][i];   // nadir point   
