...
import jmetal.util.comparators.Extreme_DominanceComparator2;
...
public class NoClusterRanking {
   ...
   */
  public Extreme_DominanceComparator2 dominance_ = new Extreme_DominanceComparator2(); // dominance.compare(,,extre)
  ...
  public NoClusterRanking(SolutionSet union, double[][] extre, double gama) {
	// TODO Auto-generated constructor stub
	  solutionSet_ = union ;
	    ...
	    if (solutionSet_.size()==0) {
			System.out.println("TWin_Ranking: The POP is empty!");
		}
	    // extreme[0]: ideal point
	    // extreme[1]: nadir point
//	    double[][] extre = new double[2][solutionSet.get(0).numberOfObjectives()]; 
//	    extre = getEXtreme(solutionSet);
	    ...
	    // of individuals that dominate me
	      ...
	      for (int q = 0; q < solutionSet_.size(); q++) {
	        ...
	        if (flagDominate == 0) {
	          flagDominate =dominance_.compare(solutionSet_.get(p),solutionSet_.get(q),extre,gama); // extreme                           
	        }
	        if (flagDominate == -1) {
	    ...
	    for (int j = 0; j < i; j++) {
	      ...
	      while (it1.hasNext()) {
	                ranking_[j].add(solutionSet_.get(it1.next().intValue()));       
	      }
	    }
